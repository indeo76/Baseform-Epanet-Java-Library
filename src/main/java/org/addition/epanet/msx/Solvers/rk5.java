...
public class rk5 {
...

    {
        ...
        while (t < tnext) {
            ...
            tnew = t + c3 * h;
            for (i = 1; i <= n; i++)
                Ynew[i] = y[i] + h * (a31 * K1[i] + a32 * K1[K2off + i]);
            jInt.solve(tnew, Ynew, n, K1, K3off, op);

            tnew = t + c4 * h;
            for (i = 1; i <= n; i++)
                Ynew[i] = y[i] + h * (a41 * K1[i] + a42 * K1[K2off + i] + a43 * K1[K3off + i]);
            jInt.solve(tnew, Ynew, n, K1, K4off, op);

            tnew = t + c5 * h;
            for (i = 1; i <= n; i++)
                Ynew[i] = y[i] + h * (a51 * K1[i] + a52 * K1[K2off + i] + a53 * K1[K3off + i] + a54 * K1[K4off + i]);
